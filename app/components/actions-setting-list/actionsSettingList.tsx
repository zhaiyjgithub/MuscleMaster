import { ChartNoAxesColumn, ChartNoAxesCombined, ChevronDown, ChevronUp, Coffee, Icon, Mountain, OctagonMinus, PersonStanding } from "lucide-react-native"
...
const ActionSettingList = ({ climbTime, stopTime, runTime, onClimbTimeChange, onStopTimeChange, onRunTimeChange }: ActionSettingListProps) => {
    const data = [
        {
            id: 1,
            color: 'orange',
    ...
    return (
        <View className="flex-row items-center justify-between">
            {data.map(({ id, icon, value, onAdd, onSubtract, color, }, index) => (
                <View key={index} className="flex-row items-center justify-center gap-x-2">
                    ...
                    <View className="flex-col gap-y-4">
                        <TouchableOpacity onPress={onAdd} className="bg-black rounded-full items-center justify-center w-10 h-10">
                            <ChevronUp size={24} color={'white'} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={onSubtract} className="bg-black rounded-full p-2 items-center justify-center w-10 h-10">
