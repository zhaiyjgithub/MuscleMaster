  ...
  return (
    <View className="flex flex-col p-3.5 rounded-xl bg-gray-100 mb-3">
      ...
      <View className="flex-row items-start mb-4">
        <View
          className="w-[36px] h-[36px] bg-white rounded-full justify-center items-center mr-2.5 flex-shrink-0"
          style={{backgroundColor: 'white'}}>
          <Bluetooth size={24} color={iconColor} />
        </View>

        <View className="flex-1">
          <Text className="font-medium mb-0.5 text-[15px] text-gray-900">
            {name}
          </Text>
          <View className="flex-row items-center">
            <Text style={{color: iconColor}} className="mr-1">
              •
            </Text>
      ...
      <View className="flex-row justify-between items-center mb-3">
        <View className="flex-row items-center">
          {[...Array(4)].map((_, index) => (
            <View
              key={index}
              className={`w-[3px] rounded-sm mr-0.5 ${
                index < activeBars ? 'bg-green-600' : 'bg-gray-300'
              }`}
              style={{
                height: 6 + index * 4,
              }}
      ...
      {connected && serviceInfos.length > 0 && (
        <View className="mt-3 border-t border-gray-200 pt-3">
          <Text className="font-medium text-[14px] text-gray-900 mb-2">
            Services:
          </Text>
              ...
              <View key={service.uuid} className="mb-2">
                ...
                {expandedService === service.uuid && (
                  <View className="ml-3 mt-1 mb-2">
                    <Text className="text-[12px] font-medium text-gray-700 mb-1">
                      Characteristics:
                    </Text>
                    {service.characteristicInfos &&
                    service.characteristicInfos.length > 0 ? (
...
const ScanFoundDeviceList: React.FC<ScanFoundDeviceListProps> = ({
  ...
  // 跟踪正在加载的设备 ID
  const [loadingDeviceIds, setLoadingDeviceIds] = useState<Set<string>>(
    new Set(),
  );
