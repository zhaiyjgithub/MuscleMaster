    ...
    const appStateListener = AppState.addEventListener(
        ...
        async nextAppState => {
          ...
          if (
            ...
            // 发送getdevice info 指令, 重新同步所有设备信息
            Object.keys(deviceConnectionStates).forEach(deviceId => {
              BLEManager.writeCharacteristic(
                  deviceId,
                  BLE_UUID.SERVICE,
                  BLE_UUID.CHARACTERISTIC_WRITE,
                ...
                console.log('Successfully get device info for device:', deviceId);
              }).catch(error => {
                console.error('Error get device info for device:', deviceId, error);
              });
            });
          } else if (
            ...
            // 停止所有计时器
            Object.keys(deviceTimerIntervalsRef.current).forEach(deviceId => {
              const interval = deviceTimerIntervalsRef.current[deviceId];
              if (interval) {
                clearInterval(interval);
                deviceTimerIntervalsRef.current[deviceId] = null;
